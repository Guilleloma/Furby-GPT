...
def record_and_transcribe(duration=8, fs=44100, threshold=1000):
    ...
    #GRABA
    print('Recording...')
    print("Abriendo stream de audio (sounddevice)")
    myrecording = sd.rec(int(duration * fs), samplerate=fs, channels=1)
    sd.wait() #Esperamos a que termine para que se cierre el stream
    print("Cerrando stream de audio (sounddevice)")
    print('Recording complete.')

    # Comprueba si el audio grabado es principalmente silencio
    if np.abs(myrecording).mean() < threshold:
        print('Detectado silencio, yendo a dormir...')
        return "a dormir" #Se va a dormir si es silencio
    
    #Genera el archivo de audio
    print('Generando archivo myrecording.wav')
    filename = 'myrecording.wav'
    sf.write(filename, myrecording, fs)
    

    #MANDA A WISPER (OPENAI) PARA SPEECH TO TEXT
    attempt = 0
    while attempt < 3: #Reintentos en caso de fallo de conexion con la API
        try:
            ...
            return transcription #Devuelve el resultado de la transcripcion
        except openai.error.APIError as e:
            print(f"Error al transcribir: {e}. Reintentando...")
            attempt += 1
